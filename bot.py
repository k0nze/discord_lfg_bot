...
lfg_command = config.get('lfg_command')


async def send_command_error_message(member, additional_content):
    command_synopsis = config.get('command_synopsis')
    ...
    dm_command_hint = config.get('dm_command_hint')
...
async def on_ready():
    ...
    if len(lfg_bot.guilds) > 0:
        ...
        for guild in lfg_bot.guilds:
            ...
            lfg_channel = guild.get_channel(config.get('lfg_channel_id'))
            message_purger = MessagePurger(config, lfg_channel)
...
async def add_id(ctx):
    ...
    if len(message.role_mentions) < 1:
        dm_no_role_mentioned = config.get('dm_no_role_mentioned')
    ...
    if len(lfg_message) == 0:
        dm_no_lfg_message = config.get('dm_no_lfg_message')
    ...
    if author_voice_channel is None:
        dm_not_in_voice_channel = config.get('dm_not_in_voice_channel')
    ...
    voice_channel_invite = await author_voice_channel.create_invite(max_age=int(config.get('invite_max_age')))
...
async def on_message(message):
    ...
    if not isinstance(channel, discord.DMChannel) and not message.author.bot:
        ...
        if (channel.id == config.get('lfg_channel_id')) and not (command_prefix + lfg_command in message.content):
